...
Monster::Monster(std::vector<int> path)
{
    ...
    circle = new Circle;
    circle->x = (path.front() % length) * grid_width + grid_width/2;
    circle->y = (path.front() / length) * grid_height + grid_height/2;
...
bool Monster::Move()
{
    ...
    if(step + 1 < path.size())
    {
        ...
        target_grid_x = (path[step] % length) * grid_width + grid_width/2;
        target_grid_y = (path[step] / length) * grid_height + grid_height/2;
        ...
        if(circle->x == target_grid_x && circle->y == target_grid_y)
        {
            ...
            switch(direction)
            {
                case LEFT:
                case RIGHT:
                    if(next_grid == cur_grid - length)
                        direction = UP;
                    else if(next_grid == cur_grid + length)
                        direction = DOWN;
